...
export default function Analyse({ params }: { params: { id: string } }) {
  const act = activites.find((a) => a.id === params.id);
  if (!act) notFound();
  const athlete = athletes.find((a) => a.id === act.athleteId);

  return (
    <div className="bg-encre -m-6 min-h-screen p-6 flex flex-col gap-5">
      <Link
        href={athlete ? `/coach/athletes/${athlete.id}` : "/coach/analyse"}
        className="font-mono text-[11px] text-[#9A938B] hover:text-[#F5F2ED] w-fit"
      >
        ← {athlete ? `Retour à la fiche de ${athlete.nom}` : "Retour aux analyses"}
      </Link>
      <div>
        <div className="font-mono text-[11px] tracking-widest uppercase text-[#9A938B]">
          {athlete?.nom} · {act.date}
        </div>
